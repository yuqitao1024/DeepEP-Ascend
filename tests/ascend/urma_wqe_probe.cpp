...
void check_words(const Request& request, const std::array<std::uint32_t, WordCount>& expected) {
    ...
    for (std::size_t index = 0; index < WordCount; ++index) {
        if (actual[index] != expected[index]) {
            std::cerr << "word " << index << ": actual=0x" << std::hex << actual[index] << " expected=0x" << expected[index] << std::dec
                      << '\n';
            ++failures;
        }
    }
...
void check_write() {
    const auto request =
        urma::make_write(make_sq(), make_remote_memory(), 3, 0x1111222233334444ULL, 0xaaaabbbbccccddddULL, 0x80, 0x13572468);
...
void check_inline_write64() {
    const auto request = urma::make_inline_write64(make_sq(), make_remote_memory(), 7, 0x1111222233334444ULL, 0x0badf00dcafebeefULL);
...
void check_faa64_and_wrap() {
    const auto request =
        urma::make_faa64(make_sq(), make_remote_memory(), 7, 0x1111222233334444ULL, 0x5555666677778888ULL, 0xfffffffffffffff7ULL, 0, 0);

    const std::array<std::uint32_t, 32> expected = {
        0xb02d0007, 0x00000b00, 0x01abc123, 0x00054321, 0x89abcdef, 0x01234567, 0x76543210, 0xfedcba98, 0x89abcdef, 0x00000000, 0x33334444,
        0x11112222, 0x00000008, 0x00000000, 0x77778888, 0x55556666, 0xfffffff7, 0xffffffff, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
