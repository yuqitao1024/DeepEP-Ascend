...
constexpr std::uint64_t kHiddenElements = 4;
...
bool check_record(
    ...
    const std::uint64_t weight_end =
        kHiddenBytes + kNumTopk * sizeof(float);
...
int main() {
    ...
    valid = check_record(
        ...
        rank_one_token_one, layout.combine_record_bytes) && valid;
    if (!valid)
        return 1;
    ...
    if (!check_record(
        ...
        return 4;
    combine_fill_normal_record_routing_weights(
        rank_one_source_weights, 0, 1, kNumTopk, nullptr, kHiddenBytes);
    return 0;
}
