...
TOPOLOGY_PROBE = ROOT / "tests/ascend/topology_contract_probe.cpp"
TRANSPORT = ROOT / "csrc/backends/ascend/transport"


class AscendTransportContractTest(unittest.TestCase):
    def test_versioned_topology_and_row_major_mapping(self):
        with tempfile.TemporaryDirectory() as directory:
            binary = pathlib.Path(directory) / "topology_contract_probe"
            compile_result = subprocess.run(
                ["c++", "-std=c++17", f"-I{ROOT}", str(TOPOLOGY_PROBE),
                 "-o", str(binary)],
