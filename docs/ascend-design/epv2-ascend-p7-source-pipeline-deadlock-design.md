...
## 11. Current Validation Snapshot (2026-09-20)

The CANN 9.3.0 source-tree snapshot and the extension used for this check were:

```text
host            NPU8P-ALT
CANN            /data/disk2/cann_version/0916/use_cann/cann-9.3.0
source tree     /home/pyptouser/yuqitao/deepep-93-work/repo
extension       deep_ep/_C.cpython-310-aarch64-linux-gnu.so
extension md5   10e626afc2ffcc6d031a9cf410bdff87
```

The focused two-rank workload was:
...
The run with the source pipeline disabled completed the case and wrote
`/tmp/source-pipeline-2r-baseline.json`. Each enabled run failed during the
first normal Dispatch preparation with the same corrupt metadata signature:

| Configuration | Chunks | Result | Failure |
| --- | ---: | --- | --- |
| macro unset | 0 | passed | none |
| `CHUNK_TILES=1024` | 2 | failed | rank 0: `actual=0, local=1, prefix=975332453`; rank 1: `actual=5301, local=0, prefix=975332453` |
| `CHUNK_TILES=512` | 4 | failed | the same `prefix=975332453` mismatch |
| `CHUNK_TILES=256` | 8 | failed | the same `prefix=975332453` mismatch |

These runs did not reproduce the historical timeout/self-deadlock: every
enabled run eventually returned. The failure moved to correctness. The
identical prefix sentinel across two, four, and eight chunks means the
source-pipeline path still does not publish or observe the required
prefix/count metadata; it is not a chunk-count-specific race.

The eight-rank performance A/B was not run. Functional correctness failed
first, and the task queue's eight devices were occupied by another CI runner at
the end of the functional check. Performance numbers collected from a run that
fails prefix/count validation would not be valid evidence of overlap benefit.

Current conclusion:

1. the historical source-pipeline deadlock is not observed in this snapshot;
2. the source-pipeline feature remains non-production and cannot be enabled;
3. `DEEP_EP_ASCEND_DISPATCH_PIPELINE_CHUNK_TILES` must not be advertised as an
   available performance knob until all functional gates in section 8 pass;
4. no performance benefit can be claimed from the 2026-09-20 snapshot.

## 12. Follow-up Probe Results (2026-09-21)

The two hypotheses that placed the failure at stream boundaries were tested on
the same host, CANN tree, extension source, and 2-rank representative workload.

| Probe | Change | Result |
| --- | --- | --- |
| release-to-producer event dependency | record an event after the persistent release launch and make the producer stream wait for it | unchanged: both ranks still report `prefix=975332453` |
| single-stream serialization | enqueue persistent release, persistent producer, and epilogue on the producer stream | unchanged: the same failure and sentinel |
| scalar-prefix probe | force the scalar `direct_dispatch_epilogue_prefix` implementation instead of parallel prefix | unchanged: the same failure and sentinel |

These probes rule out a missing host event between the communication and
producer streams as the primary cause. They also rule out the choice between
the current scalar and parallel expert-prefix kernels. The persistent
producer/release protocol itself, or one of its state/argument transitions,
remains the leading suspect.

The failure signature is informative: rank 0 observed `actual=0` for expert 0,
rank 1 observed `actual=5301`, and both observed the same
`prefix=975332453` (`0x3a226465`). Thus the expert-count path produced values
consistent with the workload, while the expert-prefix bridge retained the
identical stale value. The next investigation should therefore capture, in one
diagnostic pass:

1. `DispatchPipelineState::diagnostic_stage/detail`, worker states,
   `completed_chunks`, and both completion generations after the failed
   launch;
2. the full `kernel_count_bridge` buffer, not only the first mismatching
   expert;
3. whether the prefix kernel executed before or after host synchronization by
   temporarily initializing the bridge with a distinct sentinel; and
4. the request/consumed counters of both persistent slots after release.

All probes above were temporary diagnostic changes and were reverted. The
remote source and extension were restored to the original
`dispatch.asc` checksum after the last probe.
